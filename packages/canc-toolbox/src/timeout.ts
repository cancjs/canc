import { ICancelable, PromiseImpl } from '@cancjs/promise';
...
function isCancelable(value: unknown): value is ICancelable {
	const candidate = value as ICancelable | null | undefined;
 ...
 * timeout wins the race, the underlying promise is canceled (if it is cancelable) so it stops its
 * own work instead of running detached. The timer is always cleared once the race settles.
 */
export function timeoutFactory(boundImpl?: PromiseImpl) {
	return function timeout<T>(promise: T | PromiseLike<T>, ms = Infinity, options?: IToolboxOptions): Promise<T> {
		const Impl = resolveImpl(options, boundImpl);

		// The returned promise owns the timer so that canceling it (CancelablePromise flavor) clears
		// the pending timeout and stops the underlying operation, leaving no leaked work. Under a
		// plain native Promise handleCancel is undefined and the timer simply runs to completion.
		return construct<T>(Impl, (resolve, reject, handleCancel?: THandleCancel) => {
			...
			const id = setTimeout(() => {
				...
				if (isCancelable(promise)) {
					(promise as ICancelable).cancel(new TimeoutError());
				}
...
export const timeout = timeoutFactory();

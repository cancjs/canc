...
describe('debounce', () => {
	...
	it('trailing: invokes fn once after quiet period', async () => {
		jest.useFakeTimers();
		let callCount = 0;
		const fn = (x: number) => { callCount++; return CancelablePromise.resolve(x); };
	...
	it('leading + trailing: first invokes, last invokes after quiet', async () => {
		jest.useFakeTimers();
		const calls: number[] = [];
		const fn = (x: number) => { calls.push(x); return CancelablePromise.resolve(x); };
	...
	it('maxWait: forces invocation even with continuous calls', async () => {
		jest.useFakeTimers();
		let callCount = 0;
		const fn = () => { callCount++; return CancelablePromise.resolve(callCount); };
	...
	it('.cancel() clears timer and rejects pending with CancelError', async () => {
		jest.useFakeTimers();
		let callCount = 0;
		const fn = () => { callCount++; return CancelablePromise.resolve('done'); };
		const debounced = debounce(fn, 100);
	...
	it('leading:false + trailing:false: fn never called', async () => {
		jest.useFakeTimers();
		let callCount = 0;
		const fn = () => { callCount++; return CancelablePromise.resolve(1); };

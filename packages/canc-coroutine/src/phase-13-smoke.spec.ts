...
// a cancAsync coroutine consuming a mock async iterable via cancForAwait
// cancels the SOURCE (calls its `.return()`, running its `finally`) when the coroutine itself is
// canceled. Not a substitute for the per-file matrix in coroutine-each.spec.ts; this is the
// example-prereq integration check (mirrors the raw stream.next()/.done/.value pattern the
// examples currently hand-roll, which this helper replaces).

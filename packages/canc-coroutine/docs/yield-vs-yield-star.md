...
A coroutine is a generator function driven by `cancAsync`. Inside it you suspend on a promise and
resume with its resolved value. There are two ways to write that suspend point, and they differ
only in what TypeScript infers for the resumed value. The runtime behavior is identical.

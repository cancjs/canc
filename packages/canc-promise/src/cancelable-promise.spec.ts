...
describe('Native Promise capture', () => {
  ...
  beforeEach(() => {
    ...
    promiseGetterSpy = jest.spyOn(global, 'Promise', 'get').mockImplementation(() => {
  ...
  it('constructs rejected promise', () => {
    const cancelablePromise = new CancelablePromise((_resolve, reject) => reject('rejected'));

    return NativePromise.resolve(cancelablePromise).then(
      () => {
  ...
  it('fulfills with .resolve()', () => {
    const cancelablePromise = CancelablePromise.resolve('resolved');
  ...
  it('rejects with .resolve()', () => {
    const cancelablePromise = CancelablePromise.resolve(
      new CancelablePromise((_resolve, reject) => reject('rejected')),
    );
  ...
  it('rejects with .reject()', () => {
    const cancelablePromise = CancelablePromise.reject('rejected');
  ...
  it('fulfills with .all()', () => {
    ...
    const cancelablePromise = CancelablePromise.all([value1, value2, value3, value4]);
...
describe('coverage completion', () => {
  it('.finally() with no callback behaves like .finally(null) (passthrough)', async () => {
    ...
    await expect(new CancelablePromise<number>((_resolve, reject) => reject('boom')).finally()).rejects.toBe('boom');
  });

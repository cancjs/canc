...
describe('combinators', () => {
	...
	describe('all()', () => {
		...
		it('respects bubble:false input — NOT canceled as a remaining loser', async () => {
			const shielded = new CancelablePromise<any>(() => {/**/}, { bubble: false });
	...
	describe('any()', () => {
		...
		it('fulfills immediately with a sync value', async () => {
			const result = CancelablePromise.any([42, new CancelablePromise(() => {/**/})]);
		...
		it('accepts mixed native + canc inputs', async () => {
			const result = CancelablePromise.any([
				NativePromise.reject(new Error('n')),
				CancelablePromise.resolve('c-win'),
			]);
		...
		it('does NOT cancel a bubble:false loser on first fulfill', async () => {
			...
			const shielded = new CancelablePromise(() => {/**/}, { bubble: false });
	...
	describe('race()', () => {
		...
		it('settles with a sync value present in the iterable', async () => {
			const result = CancelablePromise.race(['immediate', new CancelablePromise(() => {/**/})]);
			...
			const badIterable: Iterable<any> = {
				[Symbol.iterator]() {
					...
					return {
						next() {
							step++;
							if (step === 1) return { value: new CancelablePromise(() => {/**/}), done: false };
							throw boom;
						},
					};
				},
		...
		it('handles adversarial sync-then thenable', async () => {
			...
			await expect(CancelablePromise.race([syncThenable, new CancelablePromise(() => {/**/})]))
	...
	describe('cross-combinator invariants', () => {
		...
		it('nested combinator: all([any(...), race(...)]) composes', async () => {
			const inner1 = CancelablePromise.any([
				NativePromise.reject(new Error('x')),
				CancelablePromise.resolve('A'),
			]);

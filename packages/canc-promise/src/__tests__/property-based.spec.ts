...
afterEach(async () => {
	...
	if (testContext.unhandledRejections.length > 0) {
		const msg = testContext.unhandledRejections
			.map((ur: any) => `${ur.reason?.message || ur.reason}`)
			.join(', ');
...
const operationArbitrary = fc.letrec((tie: any) => ({
	operation: fc.oneof(
		fc.tuple(fc.constant('then'), fc.integer({ min: 0, max: 100 }))
			.map(([op, tick]: [string, number]) => ({ type: 'then' as const, tick })),
		fc.tuple(fc.constant('catch'), fc.integer({ min: 0, max: 100 }))
			.map(([op, tick]: [string, number]) => ({ type: 'catch' as const, tick })),
		fc.tuple(fc.constant('finally'), fc.integer({ min: 0, max: 100 }))
			.map(([op, tick]: [string, number]) => ({ type: 'finally' as const, tick })),
		fc.tuple(fc.constant('cancel'), fc.integer({ min: 0, max: 100 }))
...
describe('Property-based (fast-check) tests', () => {
	it(
		'random operation sequences maintain invariants (1000 runs)',
		async () => {

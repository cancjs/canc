...
function isDurationShaped(value: unknown): value is TDuration {
 ...
 */
export function parseTimedArgs<TInput>(rest: readonly unknown[]): ITimedArgs<TInput> {
  if (rest.length <= 1) {
    return { hasInput: false, duration: rest[0] as TDuration, options: undefined };
  }

  if (rest.length === 2 && !isDurationShaped(rest[1])) {
    // (duration, options) - the second argument is not duration-shaped, so it is the options bag.

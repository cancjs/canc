...
- **Default vs Named exports**: Default exports are provided on `@cancjs/promise` (`CancelablePromise`), `@cancjs/fetch` (`cancelableFetch`), and `@cancjs/axios` (`cancelableAxios`) because these packages act as drop-in replacements for standard default-exported entities. `toolbox`, `toolbox-native`, `coroutine`, `decorators`, and `unhandled-rejection` export named symbols only.

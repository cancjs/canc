...
export function DetailPanel({ api, id }: { api: TravelApi; id: string }): ReactNode {
 const resource = useCancelableResource(
 (getSignal) => api.destinationDetails(id, getSignal()),
 [api, id]
 );

 return (
 <CancelableSuspense resource={resource} fallback={<p style={{ color: '#aaa' }}>loading details…</p>}>
 {(details) => (

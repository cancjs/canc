...
function chatCalls(mockApi: MockApiBundle): number {
 return mockApi.api.calls.filter((c) => c.endpoint.startsWith('chat.token')).length;
}

function abortedCalls(mockApi: MockApiBundle): number {
 return mockApi.api.calls.filter((c) => c.status === 'aborted').length;
}
...
describe('rag pipeline (canc)', () => {
 ...
 expect(abortedCalls(mockApi)).toBe(0);
 });

  ...
  canc: {
    ...
    async waterfall(cancelMidFlight) {
      ...
      const flow = root.then(async (first) => {
        ...
        return CancelablePromise.all([this.request(acc), this.request(acc + 1), this.request(acc + 2)]);
      });
      if (cancelMidFlight) {
  ...
  bluebird: {
    ...
    async waterfall(cancelMidFlight) {
      ...
      const flow = root.then(async (first) => {
        ...
        return Bluebird.all([this.request(acc), this.request(acc + 1), this.request(acc + 2)]);
      });
      if (cancelMidFlight) {
        setImmediate(() => root.cancel());
      }
...
async function measureMemoryPer1k(impl) {
  ...
  cancelAll();
  await Promise.all(promises.map((p) => (p && typeof p.then === 'function' ? p.then(noop, noop) : null)));
  void promises;
  global.gc();
...
function buildSummary(result) {
  ...
  if (cMem.supported && nMem.supported) {
    ...
    mem =
      ...
      `AbortController baseline (${(cMem.kbPer1k / 1024).toFixed(1)} MB vs ${(nMem.kbPer1k / 1024).toFixed(
        1,
      )} MB per 1k).`;
  } else {
  ...
  const wPhrase =
    Math.abs(wDelta) < 5 ?
      `about even with native (${wDelta >= 0 ? '+' : ''}${wDelta.toFixed(0)}%, ~${Math.abs(wAbs).toFixed(
        2,
      )} µs per request)`
    : `${Math.abs(wDelta).toFixed(0)}% ${dir(wDelta)} native (~${Math.abs(wAbs).toFixed(2)} µs extra per request)`;
